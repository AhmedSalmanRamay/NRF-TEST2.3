...
static volatile uint8_t accel1_level = 0; 
static volatile uint8_t accel2_level = 0; 
...
static void accel_level_update(void)
{
   ...
   // uint8_t str[]="145";
   // leng = sizeof(str);
    err_code = ble_cus_accel_level_update(&m_cus, accel_level, m_conn_handle);
...
static void accel1_level_update(void)
{
    ret_code_t err_code;
   // uint8_t str[]="145";
   // leng = sizeof(str);
    err_code = ble_cus_accel1_level_update(&m_cus, accel1_level, m_conn_handle);
    if (err_code != NRF_SUCCESS &&
        err_code != BLE_ERROR_INVALID_CONN_HANDLE &&
        err_code != NRF_ERROR_INVALID_STATE &&
        err_code != BLE_ERROR_GATTS_SYS_ATTR_MISSING)
...
static void accel2_level_update(void)
{
    ret_code_t err_code;
   // uint8_t str[]="145";
   // leng = sizeof(str);
    err_code = ble_cus_accel2_level_update(&m_cus, accel2_level, m_conn_handle);
    if (err_code != NRF_SUCCESS &&
        err_code != BLE_ERROR_INVALID_CONN_HANDLE &&
        err_code != NRF_ERROR_INVALID_STATE &&
        err_code != BLE_ERROR_GATTS_SYS_ATTR_MISSING)
    ...
    for (;;)
    {
     ...
     int16_t x_accel, y_accel, z_accel;
        ADXL372_Read_Accel_Data(&x_accel, &y_accel, &z_accel);
         accel_level = x_accel;
         accel1_level = y_accel;
         accel2_level = z_accel;
          accel_level_update();
          accel1_level_update();
          accel2_level_update();
          nrf_delay_ms(250);
        ...
        SEGGER_RTT_printf(0, "Y-Axis: %d ", y_accel);
