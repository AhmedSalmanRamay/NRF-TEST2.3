...
void ADXL372_Setup() {
    // ---- I2C init process ---- //
    nrf_drv_twi_config_t m_twi_adx372_config = {
       .scl                = 22,   // Specify your SCL pin here
       .sda                = 25,   // Specify your SDA pin here
    ...
    // Initialize TWI. 
    err_code = nrf_drv_twi_init(&m_twi_adx372, &m_twi_adx372_config, NULL, NULL);
    APP_ERROR_CHECK(err_code);
    nrf_drv_twi_enable(&m_twi_adx372);
    // ----------------------------- //
...
void ADXL372_Write(uint8_t address, uint8_t value) {
    ...
    err_code = nrf_drv_twi_tx(&m_twi_adx372, ADXL372_I2C_ADDRESS, data, sizeof(data), false);
    APP_ERROR_CHECK(err_code);
    return;
}
...
uint8_t	ADXL372_Read(uint8_t address) {
    ...
    // Receive data
    ret_code_t err_code = nrf_drv_twi_tx(&m_twi_adx372, ADXL372_I2C_ADDRESS, &address, sizeof(address), true);
    APP_ERROR_CHECK(err_code); 

    uint8_t data;

    // Read data from the register
    err_code = nrf_drv_twi_rx(&m_twi_adx372, ADXL372_I2C_ADDRESS, &data, sizeof(data));
    APP_ERROR_CHECK(err_code);
...
void ADXL372_Read_Accel_Data(int16_t *x, int16_t *y, int16_t *z) {
    ...
    data[0] = ADXL372_Read(ADXL372_XDATA_L);
    data[1] = ADXL372_Read(ADXL372_XDATA_H);
    data[2] = ADXL372_Read(ADXL372_YDATA_L);
    data[3] = ADXL372_Read(ADXL372_YDATA_H);
    data[4] = ADXL372_Read(ADXL372_ZDATA_L);
    data[5] = ADXL372_Read(ADXL372_ZDATA_H);
